...
//generate unique number to avoid colission in the DB for the invoice number
...
export async function getInvoices(req, res) {
  try {
    ...
    const invoices = await Invoice.find(q).sort({ createdDate: -1 }).lean();
...
export async function updateInvoice(req, res) {
  try {
    ...
    // ensure that it not exists already in another invoice
    /* UPDATE */
    if (
      body.invoiceNumber &&
